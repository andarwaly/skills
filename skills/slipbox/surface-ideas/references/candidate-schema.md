...
| `reason` | The question + motivation surfaced for this candidate, set once at extraction time. Stays unchanged if the candidate is later dismissed at pick time in `discuss-idea` — nothing is blanked, nothing new is written. Contrast with `surface-ideas`'s own surface-time dismissals, which never create a row at all, so there's no reason to blank in the first place. |
...
This file is duplicated (not shared) with `discuss-idea`'s copy of the same schema — see that skill's own `references/candidate-schema.md` for why.
