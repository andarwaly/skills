...
- `references/candidate-schema.md` in this task is a **duplicate** of Task 2's file, not a shared pointer to it — see Task 2's "Note on schema sharing" for why (install-granularity constraint: `npx skills add` installs individual skills, not the bucket).
...
`references/candidate-schema.md` content is identical to `extract-idea`'s copy (see Task 2, Step 1) — write the same file verbatim into this skill's own `references/` folder.
...
- [ ] **Step 4: `writing-great-skills` audit** — check:
  ...
  - The CSV schema copy matches Task 2's `extract-idea` copy exactly (deliberate duplication, install-granularity constraint — not a shared reference).
